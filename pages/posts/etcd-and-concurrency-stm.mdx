...
After taking some time to better understand some of the core concepts under etcd, I've finally got my head around an issue I opened that never got any attention, but that sat in my subconscious unsolved (at-least from my own understanding perspective).
...
InfluxDB Cloud 2 (as it was known back then) was originally forked from the open-source codebase, however, it was considerably cannibalised and re-implemented particularly around this time (2022).
...
That said, it came with a lot of the abstractions originally set down in InfluxDB v2 OSS.
...
The `kv.Store` abstraction we had to work with, provided a functional transaction interface.
You provide a function, the runtime provides a client on which to performance `Get` and `Put` operations.
...
Etcd uses gRPC for communication, and their client had easy to configure support to leverage a custom etcd client-side load balancing strategy. SRE did the work to leverage this, with the goal of more even distribution of traffic to etcd.

The change was tested in a non-production environment, but guess what, the workloads there weren't quite as representative of production as one would've hoped. So the change passed with flying colours and ultimately made it to production.
...
The API was soon returning 500s and our internal task scheduling system was backing up fast.
It didn't take long for the platform to take a nose dive and a _sev 1_ incident was declared.

Fortunately, we were seasoned enough to know it was probably our own faults, so the recent changes were reverted quickly.
...
In the STM, there is an assumption which falls flat on its face when used in conjunction with client-side load balancing.
...
The kinds of conditions supported are e.g. equality or boundary comparisons on a particular keys value, as well as asserting that a key has or hasn't been modified since some revision. 
...
So **be aware readers** that your function call better be idempotent. Because it can and will be retried until the transaction condition holds true (or a non-nil error is returned).
...
This function is supplied with an implementation of the `concurrency.STM` interface we just learnt about.
It then gets two keys `"keyOne"` and `"keyTwo"` via the STM.
...
In particular, we're going to zoom into the assumption I eluded to earlier.
You see, the STM attempts to make (what I have come to assume is) an optimization.

The first time you call `Get` (read a value) the STM will actually capture the returned values store revision number.
...
However, they also sneak in one little extra cheeky predicate `WithSerializable()`.
...
So we know the root cause. You can't trust the revision returned for a linearized read to be one which has been reached by **all** replicas in the cluster.
...
I could see this the behaviour being exhibited, however, I didn't understand why the concurrency STM appeared to continue making this incorrect assumption.

Part of me wondered whether there was a bug, and the first read should only return the most up to date revision for **all** cluster members (this is not a bug).
Alternatively, maybe the etcd team just haven't gotten around to internalizing this combination of the STM with client-side load-balancing.
...
I mostly wanted clarification on my (mis)assumptions and the assumptions of the STM code, and how they felt this code should be handled (maybe it needs a warning not to use it with client-side load-balancing). 
...
```mermaid
flowchart LR
    n1("Leader: Node One (3)")
    n2("Replica: Node Two (3)")
    n3("Replica: Node Three (3)")
    c(Client)
...
```mermaid
flowchart LR
    n1("Leader: Node One (4)")
    n2("Replica: Node Two (4)")
    n3("Replica: Node Three (3)")
    c(Client)
...
flowchart LR
    ...
    n2 <---> n3
    n1 <---> n3
    c -- "Read('keyTwo', WithRev(4), WithSerializable())" --> n3
...
flowchart LR
    ...
    n2 <---> n3
    n1 <---> n3
    c -- "mvcc: required revision is a future revision" --- n3
...
The leader would've then appended the read as a command to the raft replication log and waited for quorum.
I'm unsure if linearized reads force the requested replica to sync up to the read, thereby allowing it to facilitate the request itself, or if the leader facilitates the read.
If anyone out there can fill me in here, that would be awesome.
...
If anything I said here is wrong, then I expect the internet to tell me (go for it).
